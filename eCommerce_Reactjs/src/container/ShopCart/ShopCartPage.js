...
import {
    getAllTypeShip, getAllAddressUserByUserIdService, createNewAddressUserrService
...
function ShopCartPage(props) {
    ...
    return (<section className="cart_area">
                ...
                <div className="table-responsive">
                    <table className="table">
                        ...
                        {dataCart && dataCart.length > 0 && dataCart.map((item, index) => {
                            ...
                            return (<ShopCartItem isOrder={false} id={item.id} userId={user && user.id}
                                                  productDetailSizeId={item.productdetailsizeData.id} key={index}
                                                  name={name} price={item.productDetail.discountPrice}
                    ...
                    {typeShip && typeShip.length > 0 && typeShip.map((item, index) => {
                        return (<div key={index} className="form-check">
                            <input className="form-check-input"
                                   checked={item.id === dataTypeShip.id ? true : false} type="radio"
        ...
        <VoucherModal closeModalFromVoucherItem={closeModalFromVoucherItem} price={price + (+priceShip)}
                      isOpenModal={isOpenModal}
                      closeModal={closeModal} id={user && user.id}/>
        <AddressUsersModal sendDataFromModalAddress={sendDataFromModalAddress} isOpenModal={isOpenModalAddressUser}
