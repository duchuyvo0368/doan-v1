            ...
            <TouchableOpacity onPress={() => handleClickItem(data.id)}>
                <View style={styles.wrapContent}>
                    ...
                    <View style={{ display: 'flex', paddingLeft: 10, paddingTop: 10 }}>
                        <Text>Single code: {data.id}</Text>

                        <Text>Value: {data.typeShipData.price}đ</Text>
                        <Text>Type: {data.typeShipData.type}</Text>
                        <Text>Form: {data.isPaymentOnlien == 0 ? 'Direct payment' : 'Online payment'}</Text>
                    </View>
                </View>
            </TouchableOpacity>
