...
let getAllProductUserRecommend = (limit) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let i = 0; i < res.length; i++) {
        ...
        for (let j = 0; j < res[i].productDetail.length; j++) {
          ...
          for (
            let k = 0;
            k < res[i].productDetail[j].productImage.length;
            k++
          ) {
      ...
      resolve({
        data: res.map((product) => {
          return {
            images: product.productDetail.flatMap((detail) =>
              detail.productImage.map((image) => image.image)
            ),
          };
        }),
      });
...
module.exports = {
  ...
  getAllProductUserRecommend: getAllProductUserRecommend,
